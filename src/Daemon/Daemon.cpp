...
namespace
{
  ...
  const command_line::arg_descriptor<uint32_t>    arg_UPGRADE_HEIGHT_V2  = {"UPGRADE_HEIGHT_V2", "uint32_t", CryptoNote::parameters::UPGRADE_HEIGHT_V2};
  const command_line::arg_descriptor<uint32_t>    arg_UPGRADE_HEIGHT_V3  = {"UPGRADE_HEIGHT_V3", "uint32_t", CryptoNote::parameters::UPGRADE_HEIGHT_V3};
  const command_line::arg_descriptor<uint32_t>    arg_KEY_IMAGE_CHECKING_BLOCK_INDEX  = {"KEY_IMAGE_CHECKING_BLOCK_INDEX", "uint32_t", CryptoNote::parameters::KEY_IMAGE_CHECKING_BLOCK_INDEX};
  ...
  const command_line::arg_descriptor< std::vector<std::string> > arg_CHECKPOINT  = {"CHECKPOINT", "Checkpoints. Format: HEIGHT:HASH", CryptoNote::CHECKPOINTS};
  const command_line::arg_descriptor<uint32_t>    arg_BUGGED_ZAWY_DIFFICULTY_BLOCK_INDEX  = {"BUGGED_ZAWY_DIFFICULTY_BLOCK_INDEX", "uint32_t", CryptoNote::parameters::BUGGED_ZAWY_DIFFICULTY_BLOCK_INDEX};
  ...
  const command_line::arg_descriptor<size_t>    arg_CRYPTONOTE_COIN_VERSION  = {"CRYPTONOTE_COIN_VERSION", "size_t", CryptoNote::parameters::CRYPTONOTE_COIN_VERSION};
  const command_line::arg_descriptor<uint64_t>    arg_TAIL_EMISSION_REWARD  = {"TAIL_EMISSION_REWARD", "uint64_t", CryptoNote::parameters::TAIL_EMISSION_REWARD};
  const command_line::arg_descriptor<uint32_t>    arg_KILL_HEIGHT  = {"KILL_HEIGHT", "uint32_t", CryptoNote::parameters::KILL_HEIGHT};
  ...
  const command_line::arg_descriptor<uint32_t>    arg_MIXIN_START_HEIGHT  = {"MIXIN_START_HEIGHT", "uint32_t", CryptoNote::parameters::MIXIN_START_HEIGHT};
  ...
  command_line::add_arg(desc_cmd_sett, arg_genesis_block_reward_address);
    ...
    bool r = command_line::handle_error_helper(desc_options, [&]()
    {
      ...
      else
      {
        ...
        std::cout << "Windows:   forknoted.exe --config-file configs/manicoin.conf" << std::endl;
        std::cout << "Linux/Mac:   ./forknoted --config-file configs/manicoin.conf" << std::endl;
        return false;
      }*/
      po::notify(vm);
...
{
  //if (command_line::get_arg(vm, arg_CRYPTONOTE_NAME) == "manicoin") {
