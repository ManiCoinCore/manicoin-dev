...
const command_line::arg_descriptor<uint32_t> arg_log_level = { "set_log", "", INFO, true };
  ...
  const command_line::arg_descriptor<bool>      arg_SYNC_FROM_ZERO  = {"SYNC_FROM_ZERO", "Sync from block 0. Use for premine wallet or brainwallet", CryptoNote::parameters::SYNC_FROM_ZERO};
...
int main(int argc, char* argv[]) {
  ...
  bool r = command_line::handle_error_helper(desc_all, [&]() {
    ...
    else
    {
      ...
      std::cout << "Windows:   simplewallet.exe --config-file configs/ManiCoin.conf" << std::endl;
      std::cout << "Linux/Mac:   ./simplewallet --config-file configs/ManiCoin.conf" << std::endl;
      return false;
    }
    po::notify(vm);
    return true;
